...
export default function MeetingTask(props) {
  ...
  const delayedChangeComment = useRef(
    throttle((comment) => props.changeComment(comment), 2000)
  ).current;
  const delayedChangeWeeklyTodo = useRef(
    throttle((todo) => props.changeWeeklyTodo(todo), 2000)
  ).current;
